...
int hist_hozon(char date_directory[], double k,std::vector<int> &hist) {
	...
	sprintf(hist_csv_directroy, "%shist.csv", date_directory);
...
////////////////////////ヒストグラムの作成及び閾値の設定////////////////////////////////////////////////////////////////////////
double discriminantAnalysis(char date_directory[], int &image_x, int &image_y,double **edge_st){
	...
	double **tmp = matrix(0, 1, 0, image_y - 1);
  ...
  */
  ...
  // printf("hist_max=%d\n",hist_max);
   ...
   Mat hist_image= Mat(Size(276,140),CV_8UC3,Scalar(255,255,255));
	...
	sprintf(hist_directory, "%shist.bmp", date_directory);
...
double edge_st_temp(char date_directory[], int &image_xt, int &image_yt, int paramerter[], int paramerter_count, int sd,double **edge_st) {
	...
	double b= discriminantAnalysis(date_directory, image_xt, image_yt, edge_st);
