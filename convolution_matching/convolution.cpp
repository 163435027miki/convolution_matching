...
////////////////////////画像の読み取り//////////////////////////////////////////////////////////////////////////////////////////////
	...
	int convolution_kurikaeshi_start_time[8];
	int convolution_kurikaeshi_end_time[8];
...
///////////////////////フィルタの読み込み・畳み込み/////////////////////////////////////////
	for(int kernel_number=0;kernel_number<8;++kernel_number){
		...
		convolution_gaus_sobel(image_y,image_x,fs,hfs,output1,spfil1,input_bmp,magnification);
			...
			convolution_kurikaeshi_start_time[kernel_number]= msectime();
		convolution_calculation(image_y,image_x,fs,hfs,output1,spfil1,input_bmp,magnification);
		convolution_kurikaeshi_end_time[kernel_number] = msectime();
		}
		//ファイルへの書き込み
...
//////////////////////////////logの作成///////////////////////////////////////////////////////////////////////////////////
	...
	fprintf(fp_date, "ImputImage=\n%s\n", InputImage);					//入力画像
	fprintf(fp_date, "ImputImage_size：X=%d，Y=%d\n", image_x, image_y);
	fprintf(fp_date, "TemplateImage=\n%s\n", Inputtemplate);					//template画像
	fprintf(fp_date, "TemplateImage_size：X=%d，Y=%d\n", image_xt, image_yt);
	...
	for (int i = 0; i < 8; ++i) {
		fprintf(fp_date,"%d\t",convolution_kurikaeshi_start_time[i]);
		fprintf(fp_date, "%d\n", convolution_kurikaeshi_end_time[i]);
	}
	fclose(fp_date);
